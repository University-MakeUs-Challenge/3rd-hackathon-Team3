...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into user (nickname, password) VALUES (?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getNickname(), postUserReq.getPassword()}; // 동적 쿼리의 ?부분에 주입될 값
        this.jdbcTemplate.update(createUserQuery, createUserParams);
        // nickname -> postUserReq.getNickname(), password -> postUserReq.getPassword() 로 매핑(대응)시킨다음 쿼리문을 실행한다.
        // 즉 DB의 User Table에 (nickname, password)값을 가지는 유저 데이터를 삽입(생성)한다.
    ...
    public int checkNickname(String nickname) {
        ...
        String checkNicknameParams = nickname; // 해당(확인할) 이메일 값
        return this.jdbcTemplate.queryForObject(checkNicknameQuery,
                int.class,
                checkNicknameParams); // checkNicknameQuery, checkNicknameParams를 통해 가져온 값(intgud)을 반환한다. -> 쿼리문의 결과(존재하지 않음(False,0),존재함(True, 1))를 int형(0,1)으로 반환됩니다.
    }

    // 회원정보 변경
    public int modifyUserName(PatchUserReq patchUserReq) {
        String modifyUserNameQuery = "update user set nickname = ? where idx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select idx, nickname, password, status from user where nickname = ?"; // 해당 nickname을 만족하는 User의 정보들을 조회한다.
        String getPwdParams = postLoginReq.getNickname(); // 주입될 nickname값을 클라이언트의 요청에서 주어진 정보를 통해 가져온다.
    ...
    public GetUserRes getUser(int userIdx) {
        String getUserQuery = "select * from user where idx = ?"; // 해당 userIdx를 만족하는 유저를 조회하는 쿼리문
